<template>
	<!-- :style='{filter:"brightness("+$store.getters.luminance+")"}' -->
  <div  class="setting" >
    <div class="set-title">
      <h2>设置</h2>
    </div>
	...
	<div class="box">
	  <div class="list">
	    <span class="name">网页亮度</span>
	    <div class="block">
			<el-slider  @input="gamma()" style="width: 250px;" input-size='large' v-model="$store.state.luminance"></el-slider>
		</div>
	  </div>
	</div>
...
export default {
  ...
  methods: {
    changeTheme(theme) {
      this.theme = theme;//改变选中不做svg加了v-if作为对号显示的判断
